...
export async function POST(req: NextRequest) {
  ...
  if (!canUser(session.user, PERMISSIONS.CLIENTS_CREATE) && !canUser(session.user, PERMISSIONS.CLIENTS_UPDATE)) {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const siteLocation = String(getVal(row, "siteLocation") ?? "").trim();
    const building = String(getVal(row, "building") ?? "").trim();
    const taxNumber = String(getVal(row, "taxNumber") ?? "").trim();
    const defaultPaymentTerms = String(getVal(row, "defaultPaymentTerms") ?? "").trim();
    const agreedDueDays = toIntOrNull(getVal(row, "agreedDueDays"));
      ...
      if (existing) {
        await prisma.client.update({
          ...
          data: {
            ...
            agreedDueDays: agreedDueDays ?? null,
            creditLimit: creditLimit ?? null,
            updatedById: userId ?? undefined,
          },
        });

        results.push({ rowNumber, identifier, status: "SUCCESS", message: "Updated client" });
      } else {
        ...
        results.push({ rowNumber, identifier, status: "SUCCESS", message: "Created client" });
      }
    } catch (e) {

...
import { getSalesReportData } from "@/app/(dashboard)/reports/actions";
  ...
  if (format === "xlsx") {
    ...
    return new NextResponse(buf, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="sales-report-${data.filters.from}-to-${data.filters.to}.xlsx"`,
      },
    });
  }
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="sales-report-${data.filters.from}-to-${data.filters.to}.csv"`,
      },
    });
  }

...
export default async function SalesReportPage({
  ...
  const xlsxHref = `/api/reports/sales?format=xlsx&from=${encodeURIComponent(data.filters.from)}&to=${encodeURIComponent(data.filters.to)}`;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Detailed invoices with drill-down links</p>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                {data.rows.length === 0 ? (
                  ...
                  <TableRow key={r.id}>
                    <TableCell><Link className="text-primary hover:underline" href={`/sales/${r.id}`}>{r.invoiceNo}</Link></TableCell>
                    <TableCell>{r.invoiceDate}</TableCell>
                    <TableCell><Link className="text-primary hover:underline" href={`/clients/${r.clientId}`}>{r.clientName}</Link></TableCell>

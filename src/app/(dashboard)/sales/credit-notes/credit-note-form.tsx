...
export function CreditNoteForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result && "error" in result && result.error) {
      ...
      setError(err.creditNoteNo?.[0] || err.salesInvoiceId?.[0] || err._form?.[0] || "Validation failed");
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        <Label htmlFor="creditNoteNo">Credit Note Number *</Label>
        <Input id="creditNoteNo" name="creditNoteNo" required maxLength={50} />
      </div>
      ...
      <div className="flex gap-4">
        <Button type="submit" disabled={submitting}>
          {submitting ? "Posting..." : "Create Credit Note"}
        </Button>

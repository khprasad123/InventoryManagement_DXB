...
import { approveQuotation, rejectQuotation } from "./actions";
import { useRouter } from "next/navigation";
import { Check, X } from "lucide-react";

export function ApproveRejectQuotation({ quotationId }: { quotationId: string }) {
  ...
  async function handleApprove() {
    setError(null);
    setLoading(true);
    const result = await approveQuotation(quotationId, remarks);
    setLoading(false);
    if (result?.error) {
  ...
  async function handleReject() {
    ...
    setLoading(true);
    const result = await rejectQuotation(quotationId, remarks);
    setLoading(false);
    if (result?.error) {
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="e.g. Approved for pricing"
              />
            </div>
      ...
      <Dialog open={open === "reject"} onOpenChange={(o) => { setOpen(o ? "reject" : null); setError(null); setRemarks(""); }}>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Rejection reason (remarks) is required. Rejected quotations cannot be amended.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="reject-remarks">Rejection reason (required)</Label>
              <Input
                id="reject-remarks"
                value={remarks}
                onChange={(e) => setRemarks(e.target.value)}
                placeholder="e.g. Pricing not approved"
                required
              />
            </div>
            {error && <p className="text-sm text-destructive">{error}</p>}
